...
const TableManagement: React.FC<TableManagementProps> = ({ tables, setTables }) => {
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 p-6 card-hover">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white gradient-text">Table Management</h2>
              <p className="text-slate-600 dark:text-slate-400">Manage restaurant tables and their status</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
          ...
          <div className="bg-gradient-to-br from-red-50 to-rose-50 dark:from-red-900/20 dark:to-rose-900/20 p-4 rounded-xl border border-red-200 dark:border-red-700">
            ...
            <div className="text-sm text-red-700 dark:text-red-400">Occupied</div>
          </div>
          <div className="bg-gradient-to-br from-yellow-50 to-amber-50 dark:from-yellow-900/20 dark:to-amber-900/20 p-4 rounded-xl border border-yellow-200 dark:border-yellow-700">
            ...
            <div className="text-sm text-yellow-700 dark:text-yellow-400">Reserved</div>
          </div>
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-4 rounded-xl border border-blue-200 dark:border-blue-700">
            ...
            <div className="text-sm text-blue-700 dark:text-blue-400">Total Capacity</div>
          </div>
        </div>
      </div>
          ...
          <div key={table.id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 p-6 card-hover">
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400">Seats {table.capacity}</p>
                </div>
              </div>
            ...
            <div className="mb-4">
              ...
              <select
                ...
                className={`w-full px-3 py-2 rounded-xl border text-sm font-medium ${getStatusColor(table.status)}`}
              >
            ...
            <div className="text-center">
              <img
                src={`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(`${window.location.origin}/menu?table=${table.number}`)}`}
                alt={`QR Code for Table ${table.number}`}
                className="mx-auto mb-2 rounded-xl border border-slate-200 dark:border-slate-600 shadow-sm"
              />
              <p className="text-xs text-slate-500 dark:text-slate-400">QR Code for Menu Access</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-md w-full p-6 animate-fade-in">
            <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-4">Add New Table</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Table Code</label>
                <input
                  type="text"
                  value={newTable.code}
                  onChange={(e) => setNewTable(prev => ({ ...prev, code: e.target.value }))}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Seating Capacity</label>
                <input
                  type="number"
