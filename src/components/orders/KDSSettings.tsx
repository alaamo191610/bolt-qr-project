      ...
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600 text-sm">
            Control how tickets show in the kitchen and auto-advance behavior.
          </p>
        </div>
      </header>
      ...
      <div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {prefs.columns.map((c, i) => (
            <span
              key={c + i}
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full border bg-white"
            >
              {c}
              <div className="flex">
                <button
                ...
                <button
                  ...
                  className="ml-1 w-6 h-6 rounded border"
                  onClick={() =>
                    moveCol(i, Math.min(prefs.columns.length - 1, i + 1))
                  }
        ...
        <div className="mt-2 text-xs text-slate-500">
          Visible statuses (left→right). You can include/exclude any of:{" "}
          {statusOptions.join(", ")}.
        </div>
      </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Select
              ...
              onChange={(v) => {
                setPrefs((p) => ({
                  ...p,
                  soundPreset: v as KDSPrefs["soundPreset"],
                }));
                setDirty(true);
              }}
            >
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              onChange={(e) => {
                setPrefs((p) => ({
                  ...p,
                  ticketScale: Number(e.target.value),
                }));
                setDirty(true);
              }}
            />
            <span className="text-sm tabular-nums">
              {prefs.ticketScale.toFixed(2)}x
            </span>
          </div>
          <Toggle
          ...
          <div className="flex items-center gap-2">
            ...
            <Select
              ...
              onChange={(v) => {
                setPrefs((p) => ({
                  ...p,
                  colorScheme: v as KDSPrefs["colorScheme"],
                }));
                setDirty(true);
              }}
            >
      ...
      <div className="flex items-center gap-2">
        ...
        <Select
          ...
          onChange={(v) => {
            setPrefs((p) => ({
              ...p,
              ticketGrouping: v as KDSPrefs["ticketGrouping"],
            }));
            setDirty(true);
          }}
        >
      ...
      <div className="grid grid-cols-12 gap-4">
        <div className="col-span-6 flex items-center gap-2">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setPrefs((p) => ({
                ...p,
                prepTimeColors: { ...p.prepTimeColors, ok: v },
              }));
              setDirty(true);
            }}
          />
        ...
        <div className="col-span-6 flex items-center gap-2">
          ...
          <input
            ...
            onChange={(e) => {
              const v = Math.max(
                prefs.prepTimeColors.ok + 1,
                Number(e.target.value || prefs.prepTimeColors.ok + 1)
              );
              setPrefs((p) => ({
                ...p,
                prepTimeColors: { ...p.prepTimeColors, warn: v },
              }));
              setDirty(true);
            }}
          />
