...
import type { OrderFlowRules, OrderStatusKey, OrderStatusDef } from '../../order-admin/types'
import { DEFAULT_FLOW } from '../../order-admin/defaults'
import { adminService } from '../../services/adminService'

function Chip({ children, className = '' }: { children: React.ReactNode; className?: string }) {
    return <span className={`px-2.5 py-1 rounded-full text-sm border bg-white ${className}`}>{children}</span>
  }
  ...
  export default function OrderWorkflowRules({ adminId }: { adminId: string }) {
    ...
    useEffect(() => {
      ...
      ;(async () => {
        ...
        try {
          ...
          setFlow(incoming && Object.keys(incoming).length ? incoming : DEFAULT_FLOW)
          setDirty(false)
        } catch (e) {
          console.error(e)
    ...
    const updateStatus = (key: OrderStatusKey, patch: Partial<OrderStatusDef>) => {
      setFlow((prev) => ({
        ...prev,
        statuses: prev.statuses.map((s) => (s.key === key ? { ...s, ...patch } : s)),
      }))
      setDirty(true)
    }
    ...
    const toggleTransition = (from: OrderStatusKey, to: OrderStatusKey) => {
      setFlow((prev) => {
        ...
        return { ...prev, transitions: { ...prev.transitions, [from]: Array.from(nexts) as OrderStatusKey[] } }
      })
      setDirty(true)
    }
    ...
    if (loading) return <div className="p-6 rounded-xl border bg-white">Loading order rules…</div>
  
    return (
      <section className="bg-white rounded-xl border shadow-sm p-6 space-y-6">
        <header className="flex items-start justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Order Workflow Rules</h3>
            <p className="text-slate-600 text-sm">Define statuses, transitions, and SLAs. These apply across Menu, KDS, and Receipts.</p>
          </div>
        </header>
        ...
        <div className="space-y-3">
          ...
          <div className="divide-y border rounded-xl">
            {flow.statuses.map((s, i) => (
              <div key={s.key} className="grid grid-cols-12 gap-3 items-center p-3">
                <div className="col-span-2 flex items-center gap-2">
                  ...
                  <button
                    className="w-8 h-8 rounded-md border bg-white disabled:opacity-40"
                    onClick={() => reorder(i, Math.min(flow.statuses.length - 1, i + 1))}
                ...
                <div className="col-span-4 flex items-center gap-2">
                  <input
                    ...
                    value={s.label_en}
                    onChange={(e) => updateStatus(s.key, { label_en: e.target.value })}
                    placeholder="Label (EN)"
                  />
                  <input
                    className="w-full border rounded-md px-2 py-1"
                    value={s.label_ar}
                    onChange={(e) => updateStatus(s.key, { label_ar: e.target.value })}
                ...
                <div className="col-span-3 flex items-center gap-4">
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      ...
                      onChange={(e) => updateStatus(s.key, { customerVisible: e.target.checked })}
                    />
                    Customer visible
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <input type="checkbox" checked={!!s.notify} onChange={(e) => updateStatus(s.key, { notify: e.target.checked })} />
                    Notify
                  </label>
                </div>
                <div className="col-span-3 flex items-center gap-2">
                  ...
                  <input
                    ...
                    value={s.slaMin ?? 0}
                    onChange={(e) => updateStatus(s.key, { slaMin: Math.max(0, Number(e.target.value || 0)) })}
                  />
                </div>
              </div>
            ))}
                    ...
                    {statusKeys.map((to) => (
                      <td key={from + to} className="p-2 text-center">
                        {from === to ? (
                          ...
                          <input
                            type="checkbox"
                            checked={flow.transitions[from]?.includes(to as OrderStatusKey) || false}
                            onChange={() => toggleTransition(from as OrderStatusKey, to as OrderStatusKey)}
                          />
                        )}
                      </td>
                    ))}
