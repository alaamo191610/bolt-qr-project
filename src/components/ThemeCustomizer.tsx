...
const classyPresets: Array<{
  name: string; primary: string; secondary: string; accent: string; note?: string;
}> = [
...
const sameHex = (a?: string | null, b?: string | null) => !!a && !!b && normalizeHex(a) === normalizeHex(b);
...
const ThemeCustomizer: React.FC<SheetProps> = ({
  ...
  subtitle = 'Elegant palettes & refined UI',
}) => {
  ...
  const applyChanges = async () => {
    ...
    updateColors(fixed);
    try { await updateAdminTheme({ theme: fixed }); } catch (e) { console.error('Failed to save theme', e); }
            ...
            <div className="rounded-2xl border bg-white/60 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 overflow-hidden">
              <div className="px-4 py-3 flex items-center justify-between bg-primary text-white">
                ...
                <div className="flex items-center gap-2">
                  <button className="text-xs px-3 py-1.5 rounded-lg border border-white/30 hover:bg-white/10 transition">Menu</button>
                  <button className="text-xs px-3 py-1.5 rounded-lg border border-white/30 hover:bg-white/10 transition">Reserve</button>
                </div>
              </div>
