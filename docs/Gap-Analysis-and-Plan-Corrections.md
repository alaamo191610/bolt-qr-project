...
|---|---|---|---|---|
| 0 | Record the four D1 decisions: tracking expiry, takeaway entry point, POS disposition, and log retention. | Yazan + Alaa | **Alaa signed 14 Aug; pending Yazan sign-off** | Decisions recorded in `D1-Decision-Log.md`: 6h tracking token/no refresh; takeaway disabled for Release 1 (dine-in only); rotatable QR capability + session exchange per ADR 0007 (D1.3's original static-code/GET design is superseded — see that section); POS schema parked; operational logs retained 30 days and redacted, kept as a separate, long-lived business/product analytics dataset for market/expansion use. Awaiting Yazan's signature to flip to Done. |
...
| 8 | Implement bounded public order creation using the capability/table-session design in `table-capability.md` (supersedes the step-0 draft). | Yazan (backend done); Alaa (frontend handoff in progress) | **Backend capability slice done 14 Aug; frontend QR/exchange handoff and idempotency/open-order-cap/pause controls remain** | Integration: no session returns 401; valid table session creates only for its bound restaurant/table; cross-tenant/rotated/revoked session is denied. Frontend E2E for QR → session → order still needed. |
